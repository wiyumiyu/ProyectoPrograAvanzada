...
/* ============================================================
   CREACIÓN DE BASE DE DATOS (SOLO SI NO EXISTE)
   ============================================================ */
IF DB_ID('DB_AlquilerVehiculos') IS NULL
BEGIN
...
/* ============================================================
   LOGINS (SOLO SI NO EXISTEN)
   ============================================================ */

-- Login para empleados
IF NOT EXISTS (SELECT 1 FROM sys.server_principals WHERE name = 'Login_Empleado')
BEGIN
...
-- Login para jefe
IF NOT EXISTS (SELECT 1 FROM sys.server_principals WHERE name = 'Login_Jefe')
BEGIN
...
IF NOT EXISTS (SELECT 1 FROM sys.server_principals WHERE name = 'Login_AdminApp')
BEGIN
...
/* ============================================================
   CAMBIAR CONTEXTO A LA BD Y CREAR ESQUEMA (SI NO EXISTE)
   ============================================================ */

USE DB_AlquilerVehiculos; -- Cambiar el contexto a la base
GO

IF NOT EXISTS (SELECT 1 FROM sys.schemas WHERE name = 'SC_AlquilerVehiculos')
BEGIN
    EXEC ('CREATE SCHEMA SC_AlquilerVehiculos'); -- Crear el esquema principal del proyecto
END
GO


/* ============================================================
   USUARIOS EN LA BD (SOLO SI NO EXISTEN)
   ============================================================ */

IF NOT EXISTS (SELECT 1 FROM sys.database_principals WHERE name = 'User_Empleado')
BEGIN
    CREATE USER User_Empleado FOR LOGIN Login_Empleado;
END

IF NOT EXISTS (SELECT 1 FROM sys.database_principals WHERE name = 'User_Jefe')
BEGIN
    CREATE USER User_Jefe FOR LOGIN Login_Jefe;
END

IF NOT EXISTS (SELECT 1 FROM sys.database_principals WHERE name = 'User_AdminApp')
BEGIN
...
/* ============================================================
   ROLES PERSONALIZADOS DEL SISTEMA (SOLO SI NO EXISTEN)
   ============================================================ */

-- Rol empleados
IF NOT EXISTS (
    SELECT 1 FROM sys.database_principals 
    WHERE name = 'rol_empleado' AND type = 'R'
)
BEGIN
...
IF NOT EXISTS (
    SELECT 1 FROM sys.database_principals 
    WHERE name = 'rol_jefe' AND type = 'R'
)
BEGIN
...
IF NOT EXISTS (
    SELECT 1 FROM sys.database_principals 
    WHERE name = 'rol_adminApp' AND type = 'R'
)
BEGIN
...
CREATE TABLE SC_AlquilerVehiculos.T_Empleados (
    ...
    telefono VARCHAR(20),
     -- Hash estándar ASP.NET Core Identity v3 en Base64 (ej: AQAAAAEAACcQAAAAE...)
...
BEGIN
    ...
    INSERT INTO SC_AlquilerVehiculos.T_Empleados
        (nombre, correo, telefono, contrasena_hash, puesto, id_sucursal, id_rol)
    VALUES
        (@nombre, @correo, @telefono, @contrasena_hash, @puesto, @id_sucursal, @id_rol);
